...
largejet_df <- df %>% filter(Aircraft == "large jet") %>% select(-Aircraft)
lear_df <- df %>% filter(Aircraft == "smalljet") %>% select(-Aircraft)

#extract the numeric data
mat <- as.matrix(largejet_df %>% select(-Month)) #GLOBAL
#build ts object (April 2021 → start = c(2021,4))
largejet_ts_df <- ts(mat,start = c(2021, 4), frequency = 12)
...
global_ex.mx = largejet_ts_df[, "Total_Maintenance"]
...
invisible(
    capture.output(
        plotts.sample.wge(largejet_ts_df[,"Total_Maintenance"]),
        file = NULL
    )
)
...
invisible(
    capture.output(
        plotts.sample.wge(log(largejet_ts_df[,"Total_Maintenance"]+1)),
        file = NULL
    )
)
...
```{r}
global_ex.fhrs = largejet_ts_df[, "total_flight_hours"]
#A. Estimate the Trend Component
...
invisible(
    capture.output(
        plotts.sample.wge(largejet_ts_df[,"total_flight_hours"]),
        file = NULL
    )
)
...
```{r}
ccf(log(largejet_ts_df[,"Total_Maintenance"] + 1), largejet_ts_df[,"total_flight_hours"])
#Indicates a lag correlation of 9 months!
# ── 0. Setup ─────────────────────────────────────────────────────────────
log_mx <- log(largejet_ts_df[,"Total_Maintenance"] + 1) 
hrs    <- largejet_ts_df[,"total_flight_hours"]
Y      <- cbind(log_mx, hrs)
...
train_idx     <- window(largejet_ts_df, end = split_pt)
test_idx      <- window(largejet_ts_df, start = st_pt)
...
times     <- as.numeric(time(largejet_ts_df))
test_t    <- tail(times,6)
# forecasts
plot(times, largejet_ts_df[,"Total_Maintenance"], type = "o", pch = 20,
...
```{r}
Y_full   <- cbind(
  log_mx = log(largejet_ts_df[,"Total_Maintenance"] + 1), hrs = largejet_ts_df[,"total_flight_hours"])
...
actual_ts   <- ts(largejet_ts_df[,"Total_Maintenance"], start = start(largejet_ts_df), 
                  frequency = 12)
...
log_mx    <- log(largejet_ts_df[,"Total_Maintenance"] + 1)
log_parts <- log(largejet_ts_df[,"Parts"] + 1)
log_labor <- log(largejet_ts_df[,"Labor"] + 1)
...
# ts metadata
start_time <- time(largejet_ts_df)[nrow(largejet_ts_df) - nrow(Y) + 1]
Y <- ts(Y, start=start_time, frequency=frequency(largejet_ts_df))
X <- ts(X, start=start_time, frequency=frequency(largejet_ts_df))
...
actual_ts <- ts(largejet_ts_df[,"Total_Maintenance"], start=start(largejet_ts_df), frequency=12)
...
```{r}
#A. Estimate the Trend Component
smalljet.mx = lear_ts_df[, "Total_Maintenance"]
smalljet.mx.yr=aggregate(smalljet.mx,FUN=mean) #annual data
# Centered Moving Average Smoother (12 Month) AKA TREND ESTIMATION
smalljet.mx.12=ma.smooth.wge(smalljet.mx,order=12,plot=FALSE)
smalljet.mx.sm12=ts(smalljet.mx.12$smooth,start=c(2022,3),frequency=12)

#B. Remove the Trend Component from the Data
smalljet.seas.mx=smalljet.mx-smalljet.mx.sm12 #Isolate Seasonality + Noise
# end(na.omit(smalljet.mx - smalljet.mx.sm12)) #sanity check

#C. Calculate an “Average” Within- year Seasonal Component
smalljet.seas.mx.numeric=as.numeric(na.omit(smalljet.seas.mx))
# convert to a matrix (ncol= number of years)
# will give monthly means beginning with first non-na month
#set month below in ts object
smalljet.seas.mx.matrix=matrix(smalljet.seas.mx.numeric,nrow = 12)
smalljet.seas.mx.matrix.t=t(smalljet.seas.mx.matrix)
smalljet.mx.months=colMeans(smalljet.seas.mx.matrix.t, na.rm=TRUE)
# replicates the 12 monthly means for each year (12)
first_ok <- start(na.omit(smalljet.seas.mx))
end_ok   <- end(na.omit(smalljet.seas.mx))
len_ok   <- length(na.omit(smalljet.seas.mx))
smalljet.mx.seas.means=rep(smalljet.mx.months,6) 
smalljet.mx.seas.means=ts(smalljet.mx.seas.means,start=first_ok,frequency=12, end = end_ok)

#D. Find the Remaining Noise Component
smalljet.mx.noise=smalljet.mx-smalljet.mx.sm12-smalljet.mx.seas.means #Noise Estimate

#Plot Data
# Data
t_idx <- time(smalljet.mx) # 1. get the fractional‐year index
breaks_frac <- seq(min(t_idx), max(t_idx), by = 0.25) # 2. define the breaks(0.25 year)
# 3. turn into real Dates & labels
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.mx, style=1, xlab = "Year", ylab = "Maintenance Costs",
           main = "MX Data (smalljet)") +
  scale_y_continuous(breaks = pretty_breaks(n = 8),
                     labels = label_number(
                       scale  = 1/1000,        # divide raw values by 1,000
                       prefix = "$",           # put a dollar sign in front
...
# Trend
tmas_idx <- time(na.omit(smalljet.mx.sm12))
breaks_frac <- seq(min(tmas_idx), max(tmas_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(na.omit(smalljet.mx.sm12), style=1, xlab = "Year", ylab = "Maintenance Costs",
           main = "MX Data (Est. Trend)") +
  scale_y_continuous(breaks = pretty_breaks(n = 8),
                     labels = label_number(
                       scale  = 1/1000,        # divide raw values by 1,000
  ...
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
                       angle = 90, vjust = 0.5, size = 8))
# Season
tsm_idx <- time(na.omit(smalljet.mx.seas.means))
breaks_frac <- seq(min(tsm_idx), max(tsm_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.mx.seas.means, style=1, xlab = "Year", ylab = "Maintenance Costs",
           main = "MX Data (Seasonal Component)") +
  scale_y_continuous(breaks = pretty_breaks(n = 8),
                     labels = label_number(
                       scale  = 1/1000,        # divide raw values by 1,000
  ...
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
                       angle = 90, vjust = 0.5, size = 8))
# Noise
tnz_idx <- time(na.omit(smalljet.mx.noise))
breaks_frac <- seq(min(tnz_idx), max(tnz_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.mx.noise, style=1, xlab = "Year", ylab = "Maintenance Costs",
           main = "MX Data (Noise)") +
  scale_y_continuous(breaks = pretty_breaks(n = 8),
                     labels = label_number(
                       scale  = 1/1000,        # divide raw values by 1,000
...
```{r}
smalljet.fhrs = lear_ts_df[, "total_flight_hours"]
#A. Estimate the Trend Component
smalljet.fhrs.yr=aggregate(smalljet.fhrs,FUN=mean) #annual data
# Centered Moving Average Smoother (12 Month) AKA TREND ESTIMATION
smalljet.fhrs.6=ma.smooth.wge(smalljet.fhrs,order=12,plot=FALSE)
smalljet.fhrs.sm6=ts(smalljet.fhrs.6$smooth,start=c(2022,5),frequency=12)

#B. Remove the Trend Component from the Data
smalljet.seas.fhrs=smalljet.fhrs-smalljet.fhrs.sm6 #Isolate Seasonality + Noise
# end(na.omit(smalljet.fhrs - smalljet.fhrs.sm6)) #sanity check

#C. Calculate an “Average” Within- year Seasonal Component
smalljet.seas.fhrs.numeric=as.numeric(na.omit(smalljet.seas.fhrs))
# convert to a matrix (ncol= number of years)
# will give monthly means beginning with first non-na month
#ncol = years, nrow = months
smalljet.seas.fhrs.matrix=matrix(smalljet.seas.fhrs.numeric,nrow = 12)
smalljet.seas.fhrs.matrix.t=t(smalljet.seas.fhrs.matrix)
smalljet.fhrs.months=colMeans(smalljet.seas.fhrs.matrix.t, na.rm=TRUE)
# replicates the 12 monthly means for each year (5)
first_ok <- start(na.omit(smalljet.seas.fhrs))
end_ok   <- end(na.omit(smalljet.seas.fhrs))
len_ok   <- length(na.omit(smalljet.seas.fhrs))
smalljet.fhrs.seas.means=rep(smalljet.fhrs.months,6) 
smalljet.fhrs.seas.means=ts(smalljet.fhrs.seas.means,start=first_ok,frequency=12,end = end_ok)

#D. Find the Remaining Noise Component
smalljet.fhrs.noise=smalljet.fhrs-smalljet.fhrs.sm6-smalljet.fhrs.seas.means #Noise Estimate

#Plot Data
# Data
t_idx <- time(smalljet.fhrs) # 1. get the fractional‐year index
breaks_frac <- seq(min(t_idx), max(t_idx), by = 0.25) # 2. define the breaks(0.25 year)
# 3. turn into real Dates & labels
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.fhrs, style=1, xlab = "Year", ylab = "Flight Hours",
           main = "Flight Hours (smalljet)") +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) + theme(axis.text.y = element_text(size = 8)) +
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
                       angle = 90, vjust = 0.5, size = 8))
...
# Trend
tmas_idx <- time(na.omit(smalljet.fhrs.sm6))
breaks_frac <- seq(min(tmas_idx), max(tmas_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(na.omit(smalljet.fhrs.sm6), style=1, xlab = "Year", ylab = "Flight Hours",
           main = "Flight Hours (Est. Trend)") +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) + theme(axis.text.y = element_text(size = 8)) +
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
                       angle = 90, vjust = 0.5, size = 8))
# Season
tsm_idx <- time(na.omit(smalljet.fhrs.seas.means))
breaks_frac <- seq(min(tsm_idx), max(tsm_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.fhrs.seas.means, style=1, xlab = "Year", ylab = "Flight Hours",
           main = "Flight Hours (Seasonal Component)") +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) + theme(axis.text.y = element_text(size = 8)) +
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
                       angle = 90, vjust = 0.5, size = 8))
# Noise
tnz_idx <- time(na.omit(smalljet.fhrs.noise))
breaks_frac <- seq(min(tnz_idx), max(tnz_idx), by = 0.25)
breaks_date  <- as.Date(as.yearmon(breaks_frac)) 
labels_date  <- format(breaks_date, "%b %Y")
plotts.wge(smalljet.fhrs.noise, style=1, xlab = "Year", ylab = "Flight Hours",
           main = "Flight Hours (Noise)") +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) + theme(axis.text.y = element_text(size = 8)) +
  scale_x_continuous(breaks = breaks_frac, 
                     labels = labels_date) + theme(axis.text.x = element_text(
...
# 1) Combine Data to Dataframe
df <- data.frame(
  Date        = as.Date(as.yearmon(time(smalljet.mx))),
  Maintenance = as.numeric(smalljet.mx),
  FlightHours = as.numeric(smalljet.fhrs)
)
...
# seasonal maintenance (shorter)
seas_ts   <- smalljet.mx.seas.means
df_seas  <- data.frame(
  Date                  = as.Date(as.yearmon(time(seas_ts))),
  Maintenance_Seasonal  = as.numeric(seas_ts)
)
...
```{r}
# 1) Your two seasonal series, with Date index
mx_seas <- smalljet.mx.seas.means
fh_seas <- smalljet.fhrs.seas.means 
dates   <- as.Date(as.yearmon(time(mx_seas)))
...
```{r}
# ── 5. Forecast 12 months ahead & evaluate ──────────────────────────────
split_pt      <- end(na.omit(smalljet.seas.fhrs))                  # Apr-2024 cutoff
st_pt         <- c(split_pt[1] - 1, split_pt[2])
